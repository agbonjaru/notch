...
@Component({
  ...
  styleUrls: ["./subscription-sidebar.component.css"],
})
export class SubscriptionSidebarComponent implements OnInit {
  ...
  @Output() getSubscriptionDateFilter = new EventEmitter();
  @Output() getSubscriptionPaymentDateFilter = new EventEmitter();
  @Output() getClientNameFilter = new EventEmitter();
  @Output() getSubscriptionOwnerFilter = new EventEmitter();
  @Output() getSubscriptionValueFilter = new EventEmitter();
  @Output() getSubscriptionBalanceValueFilter = new EventEmitter();
  @Output() getCustomFilter = new EventEmitter();
  @Output() getClearFilter = new EventEmitter();
  @Output() getSubscriptionEndDateFilter = new EventEmitter();
  @Output() getSubscriptionCustomFilter = new EventEmitter();
  @Output() getSubscriptionCurrencyFilter = new EventEmitter();

  subscriptionDateForm = new FormGroup({
    fromDate: new FormControl("", [Validators.required]),
    toDate: new FormControl("", [Validators.required]),
  });

  subscriptionEndDateForm = new FormGroup({
    fromDate: new FormControl("", [Validators.required]),
    toDate: new FormControl("", [Validators.required]),
  });
  ...
  subscriptionOwnerForm = new FormGroup({
    owner: new FormControl("", Validators.required),
  });

  subscriptionValueForm = new FormGroup({
  ...
  createFilterForm = new FormGroup({
    ...
    subscriptionOwner: new FormControl("", Validators.required),
    subscriptionValueFrom: new FormControl(0, Validators.required),
    subscriptionValueTo: new FormControl(0, Validators.required),
  ...
  clearAllFilter() {
    ...
    this.subscriptionDateForm.reset();
    this.subscriptionValueForm.reset();
    this.clientForm.reset();
  }

  filterBySubscriptionDate() {
    this.getSubscriptionDateFilter.emit();
  }

  filterBySubscriptionEndDate() {
    this.getSubscriptionEndDateFilter.emit();
  }
  ...
  filterByCurrency() {
    this.getSubscriptionCurrencyFilter.emit();
  }

  subscriptionValueFilter() {
    this.getSubscriptionValueFilter.emit();
  }

  subscriptionOwnerFiter() {
    this.getSubscriptionOwnerFilter.emit();
  }

  customFilter(filter) {
    this.getSubscriptionCustomFilter.emit(filter);
  }
  ...
  getFilters() {
    this.customFilterServ
      .getCustomFilter(this.userId, "subscription")
      .pipe(takeUntil(this.unsubscribe))
      .subscribe((data: any) => {
