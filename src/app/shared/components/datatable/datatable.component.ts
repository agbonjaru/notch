...
export class DatatableComponent implements OnInit {
  ...
  headClicked = (headData, refElement?) => {
    switch (headData.key) {
      case "checkbox":
        ...
        document
          ...
          .forEach(
            (checkbox: HTMLInputElement) => (checkbox.checked = !checked)
          );
        if (checked) {
          this.selectedrows = [];
        } else {
      ...
      default:
        ...
        this.bodyrows.sort((a: any, b: any) => {
          ...
          let nameA = a[headData.key]
            ? (a[headData.key] + "").toUpperCase()
            : ""; // ignore upper and lowercase
          let nameB = b[headData.key]
            ? (b[headData.key] + "").toUpperCase()
            : ""; // ignore upper and lowercase
          if (nameA < nameB) {
            return -1;
          }
    ...
    if (this.headHash[headData.key].sorted) {
      el.innerHTML = `  <i class='fa  fa-caret-up'></i>`;
    } else {
      el.innerHTML = `  <i class='fa  fa-caret-down'></i>`;
    }
  };
  initializeTable = () => {
  ...
  startPaginate = (init?) => {
    ...
    if (init) {
      this.totalAvailablePagination =
        Math.ceil(this.bodyrows.length / this.paginate) || 1;
    }
  ...
  itemChecked(data, push?) {
    if (push) {
      ...
      this.selectedrows = this.selectedrows.filter(
        (rdata) => !(rdata[firstkey] === data[firstkey])
      );
    }
    this.feedback.emit({ type: "selectedrows", data: this.selectedrows });
    this.resetCheckBox();
  ...
  parseDate(number) {
    ...
    if (!parseInt(number)) {
      number = (number + "").replace("WAT", "UTC");
    }
    return number ? new Date(number).toDateString() : "";
  }
  numberizeDate(date) {
    if (!date) {
      return 0;
    ...
    if (!parseInt(date)) {
      date = (date + "").replace("WAT", "UTC");
    }
    return date ? Date.parse(new Date(date).toDateString()) : 0;
  }
  emitViewActions(data, key) {
    if (key === "checkbox" || key === "action") {
      return;
    }
    ["View/Edit", "View / Edit", "View", "Edit"].forEach((action) =>
      this.feedback.emit({ type: "singleaction", action, data })
    );
  }
  OnDestroy() {
